        ...
        <div className="w-full flex flex-col flex-nowrap justify-start mb-2">
            ...
            {showItems &&
                <ul className="w-full text-center px-[10%] py-[1%]">
                    {itemList.map((item) => {
                        return (<li className="w-full flex flex-row flex-nowrap justify-between p-[1%] bg-slate-300 border border-b-slate-300" key={item.id}>
                            <span className="p-[1%]">{item.itemName + ' Rs ' + item.cost}</span>
                            <button onClick={() => handleAddItem(item)} className="bg-slate-100 cursor-pointer text-black p-[1%]">Add Item</button></li>)
                    })}
                </ul>}
